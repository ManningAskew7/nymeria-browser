...
const UNARMED: DeliveryProbe = { read: async () => 'unknown' }
...
export async function armDelivery(
  ...
  let armed = await evaluateInWorld<boolean>(tabId, contextId, armExpression(types, id))
  if (!armed.ok && armed.contextGone) {
    ...
    if (contextId === null) return UNARMED
    armed = await evaluateInWorld<boolean>(tabId, contextId, armExpression(types, id))
  }
  ...
  let spent = false
  return {
    async read(): Promise<DeliveryOutcome> {
      if (spent) return 'unknown'
      spent = true
      const result = await evaluateInWorld<number | null>(tabId, world, readExpression(id))
      if (!result.ok) {
      ...
      if (typeof result.value !== 'number') return 'unknown'
      return result.value > 0 ? 'yes' : 'no'
    },
  }
}

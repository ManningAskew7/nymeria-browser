...
export async function execScreenshot(args: unknown): Promise<CommandResult> {
  ...
  if (wantsRegion) {
    ...
    const asked = finite(a.region_scale) ?? REGION_SCALE_DEFAULT
    const scale = Math.min(Math.max(asked, REGION_SCALE_MIN), REGION_SCALE_MAX)
    clip = { ...bounded, scale }
  }

  const resp = await sendCommand<{ data: string }>(a.tab_id, 'Page.captureScreenshot', {
    format: 'png',
    // A clip ALWAYS rides captureBeyondViewport, whatever part of the document
    // it names. Measured 2026-08-16: without it, a clip over anything not
    // currently on screen returns a successful capture of pure white, one
    // colour in every pixel, with no error anywhere. With it, the same clip
    // returns the real pixels. That also makes a region of an element below
    // the fold work without scrolling the user's page to reach it.
    captureBeyondViewport: fullPage || clip !== null,
    ...(clip ? { clip } : {}),
  })
